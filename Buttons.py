...
def get_credentials(entry, box):
    credenciais_txt = get_entry_text(entry, box)
    user = None
    password = None
    with open(credenciais_txt, 'r', encoding='utf-8') as txt:
        for line in txt:
...
def show_box():
    ...
    entry = dp.tk.Entry(box, width=40)
    entry.pack(pady=4)
    entry.insert(0, default_file_path)
    
    submit_button = dp.tk.Button(box, text='Enviar', command=lambda: on_submit(entry, box))
...
def on_submit(entry, box):
    user, password = get_credentials(entry, box)
    if user is not None and password is not None:
